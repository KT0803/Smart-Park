export default function ParkingLotCard({ lot, onBook }) {
  ...
  return (
    <div className="card flex flex-col gap-4 hover:border-blue-700 transition-colors duration-200 animate-fadein">
      {/* Header */}
      <div>
        <h3 className="text-lg font-semibold text-white">{lot.name}</h3>
        <p className="text-sm text-gray-400 mt-0.5">📍 {lot.location}</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        ...
        <div className="bg-gray-800/60 rounded-xl p-2.5">
          <p className="text-lg font-bold text-white">{lot.totalSlots}</p>
          <p className="text-xs text-gray-500">Total</p>
        </div>
        <div className="bg-gray-800/60 rounded-xl p-2.5">
          <p className="text-lg font-bold text-blue-400">₹{lot.pricePerHour}</p>
